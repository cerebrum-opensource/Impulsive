        ...
        <div class="row">
            ...
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                <div class="product-status-wrap loader_div">
                    <h4>{{ trans('label.footer_links') }}</h4>
                    <div class="add-product">
                        <a href="{{ route('add_footer_link') }}">{{ trans('label.add_footer_link') }}</a>
                    </div>
...
function changeStatus(model,action,id,text){
      /*bootbox.confirm({ 
        ...
        var r = confirm("{{ trans('message.are_you_sure_you_want_to') }} "+text+" {{ trans('message.this_link') }}");
        if (r == true) {
          $.ajax({
                url:"{{ route('change-footer-link-status') }}",
                data:{id:id,status:action},
                success:function(data){
                    window.location.reload();
                }
