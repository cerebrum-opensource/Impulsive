...
<li id="auction{{$aucId}}" class="list-view">
   ...
   <span class="view-img"><a class="add_to_recall"  id="add_to_recall{{$plannedAucId}}" onClick="javascript:addToWishlist('{{$plannedAucId}}','{{$type}}','{{$aucId}}')"><img src="{{ $watchListImage }}" id="wishlist_auction{{$plannedAucId}}" ></a></span>
   ...
   <img src="@if($image) {{ asset('/images/admin/products/').'/'.$liveAuction->product->id.'/'.$image }} @else {{ asset('images/tool.png') }} @endif"  class="box-img2" alt="{{ $liveAuction->product->product_title }}" title="{{ $liveAuction->product->product_title }}">
     ...
     <span class="alert" style="display: none;" id="recall_auction{{$plannedAucId}}" role="alert">
   ...
   <span class="text-lower list-view-text">
      ...
      <span class="text1a text3a" id="bid_price{{$aucId}}"> {{ $liveAuction->bid_price }} {{ CURRENCY_ICON }}</span>
      ...
      <div class="outer-box">
         ...
         <div class="messageTip alert-warning" id="bid_user_login{{$aucId}}" style="display: none" role="alert"> </div>
      </div>
   </span>
