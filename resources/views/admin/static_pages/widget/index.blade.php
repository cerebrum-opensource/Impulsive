        ...
        <div class="row">
            ...
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                <div class="product-status-wrap loader_div">
                    <h4>{{ trans('label.widget') }}</h4>
                    @if($count < 3)
                    <div class="add-product">
                        <a href="{{ route('add_widget') }}">{{ trans('label.add_widget') }}</a>
                    </div>
                    @endif
                    <table class="datatable ">
                        <thead>
                                <tr>
                                    ...
                                    <th>{{ trans('label.header_3') }} </th>
                                    <th>{{ trans('label.header_link') }} </th>
...
jQuery(document).ready(function() {
    ...
    $('.datatable').DataTable({
        ...
        serverSide: true,
        bSort: false,
        ajax: "{{ url('admin/widgetsList') }}",
        columns: [
            {data: 'DT_RowIndex', name: 'DT_RowIndex',orderable: false, searchable: false},
            {data: 'header1', name: 'header1',orderable: false},
            {data: 'header2', name: 'header2',orderable: false},
            {data: 'header3', name: 'header3',orderable: false},
            {data: 'header4', name: 'header4',orderable: false},
