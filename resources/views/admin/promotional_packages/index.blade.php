...
@php
  $name = trans('label.promotional_package_list');
        ...
        <div class="row">
            ...
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                <div class="product-status-wrap">
                    <h4>{{ trans('label.promotional_package_list') }}</h4>
                    <div class="add-product">
                        <a href="{{ route('add_promo_package') }}">{{ trans('label.add_promotional_package') }}</a>
                    </div>
                    <table class="datatable">
                        <thead>
                                <tr>
                                    ...
                                    <th>{{ trans('label.number_of_bid') }} </th>
...
jQuery(document).ready(function() {
    ...
    $('.datatable').DataTable({
        ...
        serverSide: true,
        bSort: false,
        ajax: "{{ route('datatable/getPromoPackages') }}",
        language: {
          searchPlaceholder: "{{trans('label.search_by_names')}}",
        },
        columns: [
...
function changeStatus(model,action,id,text){
      /*bootbox.confirm({ 
        ...
        var r = confirm("Are you sure, you want to "+text+" this package?");
        if (r == true) {
          $.ajax({
                url:"{{ route('change-promo-package-status') }}",
                data:{id:id,status:action},
                success:function(data){
                    window.location.reload();
                }
...
function promo_package_delete(package_id){
    // alert(category_id);
    // var confrm = confirm("Are you sure that you want to delete this package?");
    var confrm = confirm("{{ trans('message.are_you_sure_delete_package') }}");
    console.log(confrm);
    if(confrm == true)
    {
        $.ajax({
            url:"{{route('promo-package-delete')}}",
            type:"POST",
            data:{id:package_id},
            // processData: false,
            // contentType: false,
